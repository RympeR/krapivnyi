...
from main import *
from top_edges import Relations
...
class Application(object):
    ...
    def header_inputs(self):
        self.entity_first_input = Entry(
            self.right, width=4, textvariable=self.entity)
        self.entity_first_input.grid(row=0, column=0, padx=(10, 5), pady=10)
        self.entity_sec_input = Entry(
            self.right, width=4, textvariable=self.second_entity)
        self.entity_sec_input.grid(row=0, column=1, padx=(10, 5), pady=10)
        self.action_input = Entry(
            self.right, width=4, textvariable=self.action)
    ...
    def footer_labels(self):
        self.entitys_label = Label(
            self.entitys, text='Сущности', font='arial 15 bold', bg='#a1c1e6')
        self.entitys_label.place(x=90, y=20)

        self.actions_label = Label(
            self.actions, text='Действия', font='arial 15 bold', bg='#a1c1e6')
        self.actions_label.place(x=90, y=20)

        self.db_label = Label(self.base, text='База знаний',
                              font='arial 15 bold', bg='#a1c1e6')
        self.db_label.place(x=90, y=20)

    def vizualize(self):
        ret = messagebox.askquestion("Вершины", "Вершины отобразить цифрами?")
        print(ret)
        if ret == 'yes':
            self.scrapper.vizualize_with_num_nodes()
        elif ret == 'no':
            self.scrapper.vizualize()
    ...
    def search_relations(self):
        ...
        relations_objects = self.scrapper.decode_num_sentence(
            entity_first_num,
            entity_sec_num,
            action_num
        )
        if relations_objects == True:
            ret=messagebox.showinfo("Успех","Да")
        elif relations_objects == False:
            ret = messagebox.showerror("Ошибка", "Нет")
        elif isinstance(relations_objects, list):
            rest = []
            for relational_object in relations_objects:
                relational_object = tuple(map(str, relational_object))
                rest.append(
                    ':'.join(relational_object)
                )
            relations = Relations(rest)
